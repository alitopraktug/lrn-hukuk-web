...
import { localizeAreaCards, localizeTeamCards } from "@/lib/i18n/localize";
...
/** Türkçe alanın "_en" karşılığı doluysa onu, yoksa Türkçesini döndürür. */
const en = (d: Record<string, string>, key: string) => d[`${key}_en`] || d[key];

/**
 * / (ana sayfa) ile birebir aynı yapı — yalnızca içerik dili İngilizce. Çalışma alanları/ekip
 * kartlarının başlık/açıklaması lib/i18n/content-en.ts'teki çeviriler varsa uygulanır (yoksa TR
 * kalır). Yayınlar her zaman Türkçedir (bkz. README → Bilinen sınırlamalar).
 */
export default async function EnglishHomePage() {
  const [page, aboutPage, settings, areasRaw, teamRaw, publications] = await Promise.all([
    getPage("home"),
    getPage("about"),
    getSiteSettings(),
    getPracticeAreas(),
  ...
  const approachRows = [1, 2, 3, 4].map((i) => ({ title: en(a, `principle${i}Title`), text: en(a, `principle${i}Text`) }));
  ...
  return (
    <>
      <Hero eyebrow={en(d, "heroEyebrow")} title={en(d, "heroTitle")} lead={en(d, "heroLead")} primaryLabel={en(d, "heroPrimaryCta")} secondaryLabel={en(d, "heroSecondaryCta")} />
        ...
        <Container>
          <div className="grid grid-cols-12 md:gap-x-8 gap-y-10">
            <div className="col-span-12 flex items-center gap-4 lg:col-span-3 lg:flex-col lg:items-start lg:gap-3">
              ...
              <span className="eyebrow">{en(d, "aboutEyebrow")}</span>
            </div>
            ...
            <div className="col-span-12 space-y-5 text-[1.02rem] leading-[1.8] text-quiet lg:col-span-4 lg:pt-3">
              {splitParagraphs(en(d, "aboutBody")).map((p, i) => (
                <p key={i}>{p}</p>
              ))}
              <div className="pt-3">
                <LinkArrow href="/en/about">{en(d, "aboutLinkLabel")}</LinkArrow>
              </div>
            </div>
          </div>
        </Container>
      ...
      <Section labelledBy="areas-title" tone="surface">
        <Container>
          <SectionHeader
            ...
            id="areas-title"
            title={en(d, "practiceTitle")}
            intro={en(d, "practiceIntro")}
            action={<LinkArrow href="/en/practice-areas">View all</LinkArrow>}
          />
        ...
        <Section labelledBy="team-title">
          <Container>
            <SectionHeader
              ...
              eyebrow="Our Team"
              id="team-title"
              title={en(d, "teamTitle")}
              intro={en(d, "teamIntro")}
              action={<LinkArrow href="/en/team">All team members</LinkArrow>}
            />
            <TeamGrid members={team} className="mt-12 lg:mt-16" basePath="/en/team" locale="en" />
          </Container>
        </Section>
      ) : null}
      ...
      <Section labelledBy="pubs-title" tone="surface">
        <Container>
          <SectionHeader
            ...
            id="pubs-title"
            title={en(d, "publicationsTitle")}
            intro={en(d, "publicationsIntro")}
            action={publications.length ? <LinkArrow href="/en/publications">All publications</LinkArrow> : undefined}
          />
          <div className="mt-12 lg:mt-16">
            {publications.length ? (
              ...
              <EmptyState title="No publications yet.">{en(d, "publicationsEmpty")}</EmptyState>
            )}
          </div>
        </Container>
      </Section>
        ...
        <Container>
          <div className="grid grid-cols-12 md:gap-x-8 gap-y-10">
            ...
            <div className="col-span-12 lg:col-span-6">
              ...
              <p className="lead mt-6 max-w-xl">{en(d, "contactText")}</p>
              <div className="mt-9">
                <ButtonLink href="/en/contact" variant="light">
                  {en(d, "contactCtaLabel")}
                </ButtonLink>
              </div>
            </div>
  ...
  return (
    <div aria-hidden="true" className="on-dark relative h-[56vh] min-h-[320px] w-full overflow-hidden bg-ink text-background sm:h-[64vh] lg:h-[72vh]">
      ...
      <div className="absolute bottom-8 left-5 sm:bottom-10 sm:left-8 lg:left-12 xl:left-16">
        <p className="text-[0.66rem] font-semibold uppercase tracking-[0.26em] text-background/55">LRN Law · Ankara</p>
      </div>
    </div>
  );
}
