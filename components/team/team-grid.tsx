...
export function TeamGrid({ members, className, priorityFirst = false }: { members: TeamCardData[]; className?: string; priorityFirst?: boolean }) {
  return (
    <ul className={cn("grid grid-cols-1 gap-x-8 gap-y-14", columns(members.length), className)}>
      {members.map((m, i) => (
        <li key={m.id} className="reveal">
          <TeamCard member={m} priority={priorityFirst && i < 2} />
        </li>
      ))}
    </ul>
  );
