...
const COPY = {
  tr: { eyebrow: "Çalışma Alanı", areasCrumb: "Çalışma Alanlarımız", toc: "İçindekiler", topics: "Başlıca konu başlıkları", team: "Bu alanda çalışan ekip üyeleri", pubs: "İlgili Yayınlar", notTranslated: "Bu çalışma alanının ayrıntılı açıklaması şu an yalnızca Türkçe olarak mevcuttur." },
  en: { eyebrow: "Practice Area", areasCrumb: "Practice Areas", toc: "Contents", topics: "Key topics", team: "Team members working in this area", pubs: "Related Publications", notTranslated: "The detailed description of this practice area is currently available in Turkish only." },
} as const;
 ...
 * yapışkan bir içindekiler listesi gösterilir. `locale="en"` ile /en altında çağrılır; `translated=false`
 * ise (bu alan için henüz İngilizce çevirisi yoksa) küçük bir uyarı gösterilir.
 */
export function AreaDetailView({
  ...
  const c = COPY[locale];
  const areasPath = localizePath("/calisma-alanlari", locale);
    ...
    <>
      <PageHero
        ...
        crumbs={
          crumbs
            ? [
                { name: c.areasCrumb, path: areasPath },
                { name: area.title, path: `${areasPath}/${area.slug}` },
              ]
            : undefined
        }
      />
        ...
        <Container>
          <div className="grid grid-cols-12 md:gap-x-8 gap-y-12">
            <aside className="col-span-12 lg:col-span-3">
              {showToc ? (
                <nav aria-label={c.toc} className="lg:sticky lg:top-[calc(var(--header-h)+28px)]">
                  <p className="eyebrow mb-4">{c.toc}</p>
            ...
            <div className="col-span-12 lg:col-span-9">
              ...
              {!translated ? <p className="mb-8 max-w-[46rem] border border-line bg-surface px-4 py-3 text-[0.9rem] text-quiet">{c.notTranslated}</p> : null}

              {area.topics.length ? (
                <div className="mb-12 max-w-[46rem]">
                  <h2 className="eyebrow mb-4 !font-sans">{c.topics}</h2>
              ...
              <ul className="col-span-12 grid gap-x-8 sm:grid-cols-2 lg:col-span-9 xl:grid-cols-3">
                {area.team.map((m) => (
                  <li key={m.id} className="border-t border-foreground/25">
                    <Link href={`${teamPath}/${m.slug}`} className="group flex items-center gap-4 py-4">
          ...
          <Container>
            ...
            <ul className="mt-10 grid gap-x-8 gap-y-12 md:grid-cols-2 lg:grid-cols-3">
              {area.publications.slice(0, 3).map((p) => (
                <li key={p.id}>
                  <PublicationCard pub={p} basePath={localizePath("/yayinlar", locale)} locale={locale} />
                </li>
              ))}
            </ul>
          </Container>
